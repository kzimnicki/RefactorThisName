...
create table configuration (id bigint not null auto_increment, created datetime, updated datetime, isPhrasalVerbAdded bit not null, max integer not null check (max<=100 AND max>=0), min integer not null check (min<=100 AND min>=0), phrasalVerbTemplate varchar(255) not null, subtitleProcessor varchar(255), subtitleTemplate varchar(255) not null, textTemplate varchar(255) not null, primary key (id));
create table rootword (id bigint not null auto_increment, rootWord_id bigint not null, primary key (id), unique (rootWord_id));
create table translation (id bigint not null auto_increment, sourceLang varchar(255), sourceWord varchar(255) not null, transLang varchar(255), transWord varchar(255), primary key (id), unique (sourceLang, transLang, sourceWord));
create table user (id bigint not null auto_increment, created datetime, updated datetime, enabled bit not null, password varchar(20) not null, role varchar(255) not null, username varchar(40) not null unique, config_id bigint, primary key (id));
create table user_excludedwords (id bigint not null auto_increment, created datetime, rootWord_id bigint, user_id bigint, primary key (id));
create table user_includedwords (id bigint not null auto_increment, created datetime, rootWord_id bigint, user_id bigint, primary key (id));
create table word (id bigint not null auto_increment, frequency integer not null, value varchar(255) not null, wordType integer not null, primary key (id), unique (value, wordType));
create table wordrelation (id bigint not null auto_increment, rootWord_id bigint not null, word_id bigint not null, primary key (id), unique (word_id, rootWord_id));
